...
EntraOpsKit 0.1.9 is read-only in its built-in live Graph behavior. Its live collector calls only Microsoft Graph GET endpoints for applications and service principals, requires Application.Read.All, rejects pagination outside the expected Global Microsoft Graph host and resource path, rejects repeated pagination URIs per resource after canonicalizing equivalent relative and absolute Global Graph forms, ignores documented agent-identity placeholder objects, and never emits credential secret or certificate values.
